...
%allcomp: use all components of jade

%L_inc_total--total array weighted according to spectral content as well

%ver==1: go by spectrum
%ver==2: sum over all spectrum
function[shape_mse,mse,pow_dif,L_inc_total]  = calculate_metrics_total_allcomp(L_inc,best_i,spec_c,H,coeff_multiple,L_inc_g,power_diff,ver)

total_spots = length(L_inc_g);

%create total array
if ver==1
    L_inc_total = sum_cell(L_inc,H(:,spec_c));
else
    L_inc_total = sum_cell(L_inc,sum(H,2));
end
