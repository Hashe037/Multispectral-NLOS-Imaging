...
scene_params.cam_angs_all = (0:1:70)+scene_params.angle_fudge; %camera angles used in light field
scene_params.ang_cut = [2,71]; %min/max angle index to analyze
scene_params.vant_cut = [10,300]; %min/max vant index to analyze
...
recon_params.dis_method = "residnorm_totweighted"; %distance metric to use for reconstruction
...
msbss_params.num_clutter = 3; %how many clutter objects to remove (loops this many times)
...
mscpa_params.mscpa_vectors_remove = []; %which vectors to remove
mscpa_params.alpha_smooth = recon_params.alpha_smooth;
...
mscpa_params.nullcut = .20; %if method = 0, the percentage of power to consider
mscpa_params.vant_smooth = .01; %.01 %smoothing on Linc
mscpa_params.mscpa_dist_min = .1; %.4

lsrecon_params = struct(); %holds information about the opt precon reconstruction
