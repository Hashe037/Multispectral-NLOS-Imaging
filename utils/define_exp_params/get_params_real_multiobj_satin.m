...
recon_params = struct(); %holds information about the agnostic algorithm
% recon_params.agnostic_dis_method = "residnorm_reconweighted";%"residnorm"; %metric to use for agnostic
recon_params.dis_method = "residnorm_totweighted";%"residnorm"; %metric to use for agnostic
...
mscpa_params = struct(); %holds information about the MS-agnostic algorithm
mscpa_params.K = 2; %expected number of clutter elements to decide nullspace size
mscpa_params.dis_method = 'residnorm_totweighted'; %'residnorm_reconweighted' %which distance metric to use
mscpa_params.vant_smooth = .0025; %.01 %smoothing on Linc
mscpa_params.attenuate_ondist = 1; %whether to attenuate Linc w.r.t. distance metric
mscpa_params.dalpha_smooth = .1;
...
mscpa_params.nullcut = .01; %.01 %if method = 0, the percentage of power to consider
mscpa_params.mscpa_dist_min = .7; %.7
mscpa_params.mscpa_vector_num = 5; %number of SVD vectors to use
