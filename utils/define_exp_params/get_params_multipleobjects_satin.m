...
scene_params.ang_cut = [2,71]; %min angle index to analyze
scene_params.vant_cut = [10,300]; %max angle index to analyze
scene_params.num_specs = 6; %number of spectrums being used
scene_params.back_sig = 0;
...
scene_params.account_responsivity = 0;
...
recon_params.dis_method = "residnorm"; %metric to use for DFOV
...
recon_params.Sinc_smooth_param = .01; %smooth the resulting lincs or not
recon_params.abs_Sincs = 0; %take absolute value of lincs or not
recon_params.alpha_smooth = .1;%.01; %parameter to smooth dalpha
...
msbss_params.num_clutter = 1; %how many clutter objects to remove (loops this many times)
...
mscpa_params.mscpa_vectors_remove = []; %number of SVD vectors to use
mscpa_params.K = 1; %expected number of clutter elements to decide nullspace size
...
mscpa_params.vant_smooth = .01; %.01 %smoothing on Linc
...
mscpa_params.nullcut = .1; %if method = 0, the percentage of power to consider
mscpa_params.mscpa_dist_min = .6; %.4
